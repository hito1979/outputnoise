...
int main() {
    ...
    for (int i = 0; i < num_sample; i++) {
        ...
        CV_v(a, b, scale, shift, num_pow, T, k_u, k_v, k_w, k_x, step, dt, epsilon, D, num_peak, start_time, distance, sample_period, repetition, base_seed, &cv_v, &cv_x);

        printf("index=%d, CV_v=%.6lf, CV_x=%.6lf\n", i, cv_v, cv_x);
