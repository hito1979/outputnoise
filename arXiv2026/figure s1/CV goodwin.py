...
def calculate_amplitude(peaks, signal):
    if len(peaks) < 2:
        return np.nan
    amplitudes = []
    for i in range(1, len(peaks)):
        segment = signal[peaks[i-1]:peaks[i]]
        amplitude = np.max(segment) - np.min(segment)
        amplitudes.append(amplitude)
    return np.mean(amplitudes) if amplitudes else np.nan

# --- Simulation function (T restored) ---
def run_simulation(k, K, d, D=0, T=39.7, num_steps=15000000, measure_start=1000000):
    dt = 0.0001
    epsilon = 0.01
    m = 10  # Hill coefficient
...
d_values = np.logspace(0, 2, 21)  # from 1 to 100
n_repeat = 100  # repeats per d
...
for d in d_values:
    ...
    for _ in range(n_repeat):
        cv_w, cv_x1, cv_x2, cv_x3 = run_simulation(k, K, d, D=D, T=T)
    ...
    print(f"d={d:.2f} | n=[w:{nW}, x1:{n1}, x2:{n2}, x3:{n3}] | "
...
ax.errorbar(d_values, mean_w,  yerr=std_w,  fmt='o', capsize=3, elinewidth=1.2, label='w')
